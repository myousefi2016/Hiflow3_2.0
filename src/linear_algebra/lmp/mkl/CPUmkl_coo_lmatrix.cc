...
CPUmkl_COO_lMatrix<ValueType>::CPUmkl_COO_lMatrix ( int init_nnz, int init_num_row, int init_num_col, std::string init_name )
{
#ifdef WITH_MKL
...
template <typename ValueType>
void CPUmkl_COO_lMatrix<ValueType>::CloneFrom ( const hiflow::la::lMatrix<ValueType> &other )
{
    if ( this != &other )
    {
...
template <typename ValueType>
void CPUmkl_COO_lMatrix<ValueType>::VectorMult ( const lVector<ValueType> &invec, lVector<ValueType> *outvec ) const
{
    ...
    {
        LOG_ERROR ( "CPUmkl_COO_lMatrix<ValueType>::VectorMult unsupported in or out vector" );
        this->print ( );
        invec.print ( );
        outvec->print ( );
        exit ( -1 );
...
template <>
void CPUmkl_COO_lMatrix<double>::VectorMult ( const CPU_lVector<double> &invec, CPU_lVector<double> *outvec ) const
{
#ifdef WITH_MKL
    ...
    mkl_cspblas_dcoogemv ( &transa, &nrow, this->matrix.val, this->matrix.row,
                           this->matrix.col, ( int * ) &( this->nnz_ ), invec.buffer, outvec->buffer );
...
template <>
void CPUmkl_COO_lMatrix<float>::VectorMult ( const CPU_lVector<float> &invec, CPU_lVector<float> *outvec ) const
{
#ifdef WITH_MKL
    ...
    LOG_ERROR ( "CPUmkl_coo_matrix::VectorMult there is no float sparse matrix-vector multiplication function" );
    exit ( -1 );
...
template <typename ValueType>
void CPUmkl_COO_lMatrix<ValueType>::CastFrom ( const CPU_COO_lMatrix<double>& other )
{
...
template <typename ValueType>
void CPUmkl_COO_lMatrix<ValueType>::CastFrom ( const CPU_COO_lMatrix<float>& other )
{
    assert ( this->get_num_row ( ) == other.get_num_row ( ) );
    assert ( this->get_num_col ( ) == other.get_num_col ( ) );
    assert ( this->get_nnz ( ) == other.get_nnz ( ) );
...
template <typename ValueType>
void CPUmkl_COO_lMatrix<ValueType>::CastTo ( CPU_COO_lMatrix<double>& other ) const
{
    assert ( this->get_num_row ( ) == other.get_num_row ( ) );
    assert ( this->get_num_col ( ) == other.get_num_col ( ) );
    assert ( this->get_nnz ( ) == other.get_nnz ( ) );
...
template <typename ValueType>
void CPUmkl_COO_lMatrix<ValueType>::CastTo ( CPU_COO_lMatrix<float>& other ) const
{
    assert ( this->get_num_row ( ) == other.get_num_row ( ) );
    assert ( this->get_num_col ( ) == other.get_num_col ( ) );
    assert ( this->get_nnz ( ) == other.get_nnz ( ) );
