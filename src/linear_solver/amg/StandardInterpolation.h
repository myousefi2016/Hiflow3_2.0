...
#ifndef SRC_LINEAR_SOLVER_AMG_STANDARDINTERPOLATION_H_
#    define SRC_LINEAR_SOLVER_AMG_STANDARDINTERPOLATION_H_
    ...
    namespace AMG
    {
        ...
        class StandardInterpolation
        {
          public:
            ...
            StandardInterpolation ( Settings const& settings = Settings ( ) )
            : settings_ ( settings )
            {
            }
            ...
            void operator() ( ResultType &result, ConnectionType const& connection, MatrixType const& Af ) const
            {
                ...
                {
                    // Intersection of strong connected and coarse variables
                    IndexVector CiS;
                    ...
                    // Intersection of strong connected and fine variables
                    IndexVector FiS;
                    std::set_intersection ( connection.strong_connections[*iter_fine_cur].begin ( ), connection.strong_connections[*iter_fine_cur].end ( ),
                                            connection.fine_variables.begin ( ), connection.fine_variables.end ( ),
                                            std::back_inserter ( FiS ) );

                    // Union of CiS and all CjS (j in FiS)
                    IndexSet active_k ( CiS.begin ( ), CiS.end ( ) );
                    for ( IndexVector::const_iterator iter_j_cur ( FiS.begin ( ) ),
                          iter_j_end ( FiS.end ( ) ); iter_j_cur != iter_j_end; ++iter_j_cur )
                    {
                        std::set_intersection ( connection.strong_connections[*iter_j_cur].begin ( ), connection.strong_connections[*iter_j_cur].end ( ),
                                                connection.fine_variables.begin ( ), connection.fine_variables.end ( ),
                                                std::inserter ( active_k, active_k.end ( ) ) );
                    }

                    // Accumulate sum_aik
                    value_type sum_aik ( 0 );
                    for ( IndexSet::const_iterator iter_k_cur ( active_k.begin ( ) ),
                          iter_k_end ( active_k.end ( ) ); iter_k_cur != iter_k_end; ++iter_k_cur )
                    {
                        for ( int p ( Af.matrix.row[*iter_fine_cur] ); p != Af.matrix.row[*iter_fine_cur + 1]; ++p )
                        {
                            if ( Af.matrix.col[p] == *iter_k_cur )
                            {
                                sum_aik += Af.matrix.val[p];
                                break;
                            }
                        }
                    }
...
#endif /* SRC_LINEAR_SOLVER_AMG_STANDARDINTERPOLATION_H_ */
