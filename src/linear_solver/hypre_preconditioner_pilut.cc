...
#include "hypre_preconditioner_pilut.h"
...
namespace hiflow
{
    namespace la
    {
        ...
        template<class LAD>
        LinearSolverState HyprePreconditionerPILUT<LAD>::ApplyPreconditioner ( const VectorType& b, VectorType* x )
        {
#ifdef WITH_HYPRE
            if ( !this->GetState ( ) )
            {
                HYPRE_ParCSRPilutSetup ( this->solver_, *( this->op_->GetParCSRMatrix ( ) ), *( b.GetParVector ( ) ), *( x->GetParVector ( ) ) );

                this->SetModifiedOperator ( false );
                this->SetState ( true );
            }
            HYPRE_ParCSRPilutSolve ( this->solver_, *( this->op_->GetParCSRMatrix ( ) ), *( b.GetParVector ( ) ), *( x->GetParVector ( ) ) );
        ...
        template<class LAD>
        void HyprePreconditionerPILUT<LAD>::Clear ( )
        {
#ifdef WITH_HYPRE
