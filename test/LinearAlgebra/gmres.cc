...
struct TestFunctor
{
            ...
            const enum MATRIX_FREE_PRECOND &precond ) const
    {
        ...
        int return_value = GMRESFunctor<double>( 3 )( x, b, A, rel_eps, abs_eps, max_iter, print_level );
        EXPECT_EQ ( 0, return_value );

        // Check result
        int x_size = 3;
        int x_index[3] = { 0, 1, 2 };
        double x_values[3] = { 0., 0., 0. };
        x->GetValues ( x_index, x_size, x_values );
...
TEST ( GMRES, simple_dense )
{
    TestFunctor ( )( CPU, NAIVE, NAIVE, DENSE, NOPRECOND );
}

TEST ( GMRES, simple_csr )
{
...
TEST ( GMRES, openmp_csr )
{
    TestFunctor ( )( CPU, OPENMP, OPENMP, CSR, NOPRECOND );
}
#endif

TEST ( GMRES, simple_csr_jacobi )
{
