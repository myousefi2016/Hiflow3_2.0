...
struct TestFunctor
{
    ...
    {
        lVector<double> *b = init_vector<double>( 3, "b", platform, vector_implementation );
        b->add_value ( 0, 1.0 );
        b->add_value ( 1, -2.0 );
        b->add_value ( 2, 0.0 );

        lVector<double> *x = init_vector<double>( 3, "x", platform, vector_implementation );
        x->Zeros ( );

        lMatrix<double> *A = init_matrix<double>( 9, 3, 3, "A", platform, matrix_implementation, matrix_format );
        ...
        CHECK_CLOSE ( -0.25, x_values[0], 1e-12 );
        CHECK_CLOSE ( -1.50, x_values[1], 1e-12 );
        CHECK_CLOSE ( -0.75, x_values[2], 1e-12 );
    }
};
