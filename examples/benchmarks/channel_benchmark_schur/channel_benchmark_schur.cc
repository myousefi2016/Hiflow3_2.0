...
#include "channel_benchmark_schur.h"
...
/// @brief Assembler for preconditioner of Schur complement equation.

class PrecondSchurAssembler : private AssemblyAssistant<DIMENSION, double>
{
  private:
    const CVector* sol_prev_;
    const double nu_;
    const double dt_;
  ...
  public:
    ...
    PrecondSchurAssembler ( const CVector* sol_prev, const double nu, const double dt )
    : sol_prev_ ( sol_prev ),
    nu_ ( nu ),
    dt_ ( dt )
    {
    ...
    /// Operator for assembly of local contribution to mass matrix.
    /// Assembles \f$\int_{K}\varphi_{j}\varphi_{i}\mathrm{d}x\f$ as local contribution on element \f$K\f$.
    /// \param[in] element Element on which local assembly is performed
    /// \param[in] quadrature Quadrature rule on given Element element
    /// \param[out] lm Local matrix containing local contributions of mass matrix
            ...
            LocalMatrix& lm )
    {
        ...
        for ( int d = 0; d < DIMENSION; ++d )
        {
            ...
            evaluate_fe_function ( *sol_prev_, d, vel_ts_[d] );
        }
        ...
        for ( size_t q = 0; q != num_q; ++q )
        {
            ...
            hiflow::Vec<DIMENSION, double> vel;
            ...
            max_vel = std::max ( max_vel, norm ( vel ) );
        }
        ...
        for ( int q = 0; q < num_q; ++q )
        {
            ...
            for ( size_t var = DIMENSION; var != DIMENSION + 1; ++var )
            {
                ...
                { //loop over test functions
                    ...
                    const double phi_i = phi ( i, q, var );
                    ...
                    { //loop over ansatz functions
                        lm ( dof_index ( i, var ), dof_index ( j, var ) )
                                ...
                                * phi ( j, q, var )
                                * phi_i
                                + tau * dot ( grad_phi ( j, q, var ), grad_phi ( i, q, var ) )
                                )
                                * dJ;
                    }
                }
...
class ChannelBenchmark : public NonlinearProblem<LAD>
{
  ...
  private:
    ...
    SchurComplement<LAD> schur_;
    // solver and preconditioner for block matrix A
    HypreGMRES<LAD> solver_A_;
    HypreBoomerAMG<LAD> precond_A_;
    // solver and preconditioner for preconditioner matrix of Schur complement
    HypreBoomerAMG<LAD> precond_precond_;
    // Mappings dof->var for BoomerAMG preconditioners
    std::vector<int> dof_func_block_one_;
    std::vector<int> dof_func_block_two_;
    ...
    std::vector<int> dirichlet_dofs_pressure_;
    std::vector<double> dirichlet_values_pressure_;
...
void ChannelBenchmark::prepare ( )
{
    ...
    if ( params_["LinearSolver"]["Preconditioning"].get<bool>( ) )
    {
    ...
    // variables belonging to first (velocity) block
    std::vector<int> variables_one;
    for ( int i = 0; i < DIMENSION; ++i )
    {
        variables_one.push_back ( i );
    }

    // variables belonging to second (pressure) block
    std::vector<int> variables_two;
    variables_two.push_back ( DIMENSION );

    schur_.Init ( space_, variables_one, variables_two );

    // compute mass scaling factor
    double mass_scaling = 1.0;

    schur_.SetBlockTwoMatrixScaling ( mass_scaling );

    this->dof_func_block_one_.clear ( );
    this->dof_func_block_one_ = space_.get_dof_func ( variables_one );

    /*for ( size_t i = 0; i != this->dof_func_block_one_.size ( ); ++i )
    {
        std::cout << "DoF " << i << ": " << this->dof_func_block_one_[i] << std::endl;
    }*/

    this->dof_func_block_two_.clear ( );
    this->dof_func_block_two_ = space_.get_dof_func ( variables_two );
    /*for ( size_t i = 0; i != this->dof_func_block_two_.size ( ); ++i )
    {
        std::cout << "DoF " << i << ": " << this->dof_func_block_two_[i] << std::endl;
    }*/
}
...
#if DIMENSION ==2
    const int inflow_bdy = params_["Boundary2D"]["InflowMaterial"].get<int>( );
    const int outflow_bdy = params_["Boundary2D"]["OutflowMaterial"].get<int>( );
#else
    const int inflow_bdy = params_["Boundary3D"]["InflowMaterial"].get<int>( );
    const int outflow_bdy = params_["Boundary3D"]["OutflowMaterial"].get<int>( );
#endif
...
#    endif
    ...
    {
        PressurePrecondBC bc ( var, inflow_bdy, outflow_bdy );
        compute_dirichlet_dofs_and_values ( bc, space_, var,
                                            dirichlet_dofs_pressure_, dirichlet_values_pressure_ );
    }
}
...
void ChannelBenchmark::run_time_loop ( )
{
    ...
    if ( params_["Backup"]["Restore"].get<bool>( ) )
    {
        ...
        vec_name << "sol_" << ts_;
        sol_.ReadHDF5 ( backup_name, "backup", vec_name.str ( ) );
        sol_.Update ( );
    ...
    if ( ts_ == 0 )
    {
        ...
        std::ostringstream vec_name;
        vec_name << "sol_" << ts_;
        sol_.WriteHDF5 ( backup_name, "backup", vec_name.str ( ) );
#endif
    }
                                  ...
                                  LAD::MatrixType* out )
{
    ...
    double forcing_current = 0.0;
    this->newton_.GetForcingTerm ( forcing_current );

    // Solver for block matrix A of inner Schur complement
    this->precond_A_.SetPreconditioningParameters ( );
    this->precond_A_.SetMaxLevels ( params_["Schur"]["PrecondBlockA"]["MaxLevels"].get<int>( ) );
    this->precond_A_.SetMaxCoarseSize ( params_["Schur"]["PrecondBlockA"]["MaxCoarseSize"].get<int>( ) );
    this->precond_A_.SetCycleType ( params_["Schur"]["PrecondBlockA"]["CycleType"].get<int>( ) );
    this->precond_A_.SetNumFunctions ( DIMENSION );
    this->precond_A_.SetInterpType ( params_["Schur"]["PrecondBlockA"]["InterpType"].get<int>( ) );
    this->precond_A_.SetCoarsenType ( params_["Schur"]["PrecondBlockA"]["CoarsenType"].get<int>( ) );
    this->precond_A_.SetRelaxType ( params_["Schur"]["PrecondBlockA"]["RelaxType"].get<int>( ) );
    this->precond_A_.SetAggNumLevels ( params_["Schur"]["PrecondBlockA"]["AggNumLevels"].get<int>( ) );
    this->precond_A_.SetAggInterpType ( params_["Schur"]["PrecondBlockA"]["AggInterpType"].get<int>( ) );
    this->precond_A_.SetRelaxWt ( params_["Schur"]["PrecondBlockA"]["RelaxWt"].get<double>( ) );
    this->precond_A_.SetStrongThreshold ( params_["Schur"]["PrecondBlockA"]["StrongThreshold"].get<double>( ) );
    this->precond_A_.SetCycleNumSweeps ( params_["Schur"]["PrecondBlockA"]["NumSweeps"].get<int>( ), 1 );
    this->precond_A_.SetCycleNumSweeps ( params_["Schur"]["PrecondBlockA"]["NumSweeps"].get<int>( ), 2 );
    this->precond_A_.SetDofFunc ( this->dof_func_block_one_ );
    this->precond_A_.SetNodal ( params_["Schur"]["PrecondBlockA"]["Nodal"].get<int>( ) );
    this->precond_A_.SetSmoothNumLevels ( params_["Schur"]["PrecondBlockA"]["SmoothNumLevels"].get<int>( ) );
    this->precond_A_.SetVariant ( params_["Schur"]["PrecondBlockA"]["Variant"].get<int>( ) );
    this->precond_A_.SetOverlap ( params_["Schur"]["PrecondBlockA"]["Overlap"].get<int>( ) );
    this->precond_A_.SetDomainType ( params_["Schur"]["PrecondBlockA"]["DomainType"].get<int>( ) );
    this->precond_A_.SetSchwarzUseNonSymm ( params_["Schur"]["PrecondBlockA"]["SchwarzUseNonSymm"].get<int>( ) );
    this->precond_A_.SetReuse ( true );
    this->precond_A_.Init ( );

    this->solver_A_.InitControl
            ( params_["Schur"]["SolverBlockA"]["NumIter"].get<int>( ),
              0.,
              params_["Schur"]["SolverBlockA"]["RelativeTolerance"].get<double>( ), 1e6 );
    this->solver_A_.InitParameter ( params_["Schur"]["SolverBlockA"]["BasisSize"].get<int>( ), "RightPreconditioning" );
    this->solver_A_.SetupPreconditioner ( this->precond_A_ );
    this->solver_A_.SetReuse ( true );

    // Solver for automatic preconditioning of inner Schur complement
    this->precond_precond_.InitControl
            ( params_["Schur"]["SolverPrecond"]["NumIter"].get<int>( ),
              0.,
              params_["Schur"]["SolverPrecond"]["RelativeTolerance"].get<double>( ), 1e6 );
    this->precond_precond_.SetMaxLevels ( params_["Schur"]["PrecondPrecond"]["MaxLevels"].get<int>( ) );
    this->precond_precond_.SetMaxCoarseSize ( params_["Schur"]["PrecondPrecond"]["MaxCoarseSize"].get<int>( ) );
    this->precond_precond_.SetCycleType ( params_["Schur"]["PrecondPrecond"]["CycleType"].get<int>( ) );
    this->precond_precond_.SetRelaxType ( params_["Schur"]["PrecondPrecond"]["RelaxType"].get<int>( ) );
    this->precond_precond_.SetAggNumLevels ( params_["Schur"]["PrecondPrecond"]["AggNumLevels"].get<int>( ) );
    this->precond_precond_.SetAggInterpType ( params_["Schur"]["PrecondPrecond"]["AggInterpType"].get<int>( ) );
    this->precond_precond_.SetRelaxWt ( params_["Schur"]["PrecondPrecond"]["RelaxWt"].get<double>( ) );
    this->precond_precond_.SetNumFunctions ( params_["Schur"]["PrecondPrecond"]["NumFunctions"].get<int>( ) );
    this->precond_precond_.SetInterpType ( params_["Schur"]["PrecondPrecond"]["InterpType"].get<int>( ) );
    this->precond_precond_.SetCoarsenType ( params_["Schur"]["PrecondPrecond"]["CoarsenType"].get<int>( ) );
    this->precond_precond_.SetStrongThreshold ( params_["Schur"]["PrecondPrecond"]["StrongThreshold"].get<double>( ) );
    this->precond_precond_.SetCycleNumSweeps ( params_["Schur"]["PrecondPrecond"]["NumSweeps"].get<int>( ), 1 );
    this->precond_precond_.SetCycleNumSweeps ( params_["Schur"]["PrecondPrecond"]["NumSweeps"].get<int>( ), 2 );
    this->precond_precond_.SetSmoothNumLevels ( params_["Schur"]["PrecondPrecond"]["SmoothNumLevels"].get<int>( ) );
    this->precond_precond_.SetVariant ( params_["Schur"]["PrecondPrecond"]["Variant"].get<int>( ) );
    this->precond_precond_.SetOverlap ( params_["Schur"]["PrecondPrecond"]["Overlap"].get<int>( ) );
    this->precond_precond_.SetDofFunc ( this->dof_func_block_two_ );
    this->precond_precond_.SetReuse ( true );
    this->precond_precond_.Init ( );

    schur_.SetupSolverBlockA ( this->solver_A_ );

    schur_.SetupSolverPrecondBlockTwo ( this->precond_precond_ );

    schur_.SetLevel ( 0 );

    schur_.SetPrintLevel ( params_["Schur"]["PrintLevel"].get<int>( ) );
    schur_.InitControl
            ( params_["Schur"]["NumIter"].get<int>( ),
              0.,
              params_["Schur"]["RelativeTolerance"].get<double>( ),
              //forcing_current,
              1.e6 );

    schur_.SetupOperator ( *out );

    gmres_.SetupOperator ( *out );
    gmres_.SetupPreconditioner ( schur_ );
                                                     ...
                                                     LAD::MatrixType* out )
{
    ...
    // assemble preconditioning matrix for second block
    PrecondSchurAssembler precond_asm ( &in, nu_, dt_ );
    mass_.Zeros ( );
    global_asm_.assemble_matrix ( space_, precond_asm, mass_ );

    if ( !dirichlet_dofs_pressure_.empty ( ) )
    {
        //mass_.diagonalize_rows ( vec2ptr ( dirichlet_dofs_pressure_ ), dirichlet_dofs_pressure_.size ( ), 1. );
    }

    schur_.SetupBlockTwoMatrix ( mass_ );
}
...
void ChannelBenchmark::ApplyFilter ( LAD::VectorType& u )
{
    ...
    /*if ( !dirichlet_dofs_pressure_.empty ( ) )
    {
        u.SetValues ( vec2ptr ( dirichlet_dofs_pressure_ ), dirichlet_dofs_pressure_.size ( ),
                         vec2ptr ( dirichlet_values_pressure_ ) );
    }*/
...
class ForceIntegral : private AssemblyAssistant<DIMENSION, double>
{
  public:
    ...
    ForceIntegral ( double nu, const CVector* sol, const std::vector<int>& bdy_dofs, FORCE_TYPE type )
