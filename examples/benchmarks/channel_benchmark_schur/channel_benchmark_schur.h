...
// Turn on SUPG stabilization of momentum equation
//#define SUPG
...
class CylinderDescriptor : public BoundaryDomainDescriptor
{
  public:
    ...
    {
#    if DIMENSION == 2
        const int mat_num_ref = 14;
#    else
        const int mat_num_ref = 13;
#    endif

        if ( mat_num == mat_num_ref )
        {
...
struct PressurePrecondBC
{
    ...
    PressurePrecondBC ( int var, int inflow_bdy, int outflow_bdy )
    : var_ ( var ), inflow_bdy_ ( inflow_bdy ), outflow_bdy_ ( outflow_bdy )
    {
    }
...
class InstationaryFlowAssembler : private AssemblyAssistant<DIMENSION, double>
{
  public:
    ...
    {
        AssemblyAssistant<DIMENSION, double>::initialize_for_element ( element, quadrature );
        // initialize solution values on current element
        initialize ( );
                    ...
                    for ( int j = 0; j < n_dofs; ++j )
                    {
                        lm ( dof_index ( i, var ), dof_index ( j, var ) ) +=
                                ...
                                * (
                                phi ( i, q, var )
#    ifdef SUPG
                                + tau_ * ( dot ( vel_ts, grad_phi ( i, q, var ) ) - nu_ * trace ( H_phi ( i, q, var ) ) )
#    endif
                                ) +
...
#    ifdef SUPG
                                ( -1.0 ) * alpha1_ * nu_ * trace ( H_phi ( j, q, var ) )
                                * tau_ * ( dot ( vel_ts, grad_phi ( i, q, var ) ) - nu_ * trace ( H_phi ( i, q, var ) ) ) +
#    endif
                    ...
                    for ( int i = 0; i < num_dofs ( test_var ); ++i )
                    {
                        for ( int j = 0; j < num_dofs ( trial_var ); ++j )
                        {
                            lm ( dof_index ( i, test_var ), dof_index ( j, trial_var ) ) +=
                                    ...
                                    ( phi ( i, q, test_var ) )
#    ifdef SUPG
                                    + tau_ * ( dot ( vel_ts, grad_phi ( i, q, test_var ) ) - nu_ * trace ( H_phi ( i, q, test_var ) ) )
#    endif
                                    ) * dJ;
                        }
                    }
                ...
                for ( int i = 0; i < num_dofs ( v_var ); ++i )
                {
                    for ( int j = 0; j < num_dofs ( p_var ); ++j )
                    {
                        lm ( dof_index ( i, v_var ), dof_index ( j, p_var ) ) +=
                                wq
                                * (
                                -alpha2_ * inv_rho_ * ( phi ( j, q, p_var ) * grad_phi ( i, q, v_var )[v_var] )
#    ifdef SUPG
                                + tau_ * alpha2_ * inv_rho_
                                * grad_phi ( j, q, p_var )[v_var]
                                * ( dot ( vel_ts, grad_phi ( i, q, v_var ) ) - nu_ * trace ( H_phi ( i, q, v_var ) ) )
#    endif
                                )
                                * dJ;
                    }
...
#    ifdef SUPG
            ...
            for ( int v_var = 0; v_var < DIMENSION; ++v_var )
            {
                for ( int i = 0; i < num_dofs ( v_var ); ++i )
                {
                    for ( int v_var_2 = 0; v_var_2 < DIMENSION; ++v_var_2 )
                    {
                        for ( int j = 0; j < num_dofs ( v_var_2 ); ++j )
                        {
                            lm ( dof_index ( i, v_var ), dof_index ( j, v_var_2 ) ) +=
                                    wq
                                    * tau_rho_
                                    * inv_rho_
                                    * grad_phi ( j, q, v_var_2 )[v_var_2]
            ...
            for ( int v_var = 0; v_var < DIMENSION; ++v_var )
            {
                for ( int i = 0; i < num_dofs ( v_var ); ++i )
                {
                    lv[dof_index ( i, v_var )] +=
                            ...
                            (
                            ( vel_ns[v_var] - vel_ts[v_var] )
                            * ( phi ( i, q, v_var )
#    ifdef SUPG
                            + tau_ * ( dot ( vel_ts, grad_phi ( i, q, v_var ) ) - nu_ * trace ( H_phi ( i, q, v_var ) ) )
#    endif
                            ) +
...
#    ifdef SUPG
                            ( -1.0 ) * nu_ *
                            ( alpha1_ * trace ( hess_vel_ns_[v_var][q] )
                            + alpha3_ * trace ( hess_vel_ts_[v_var][q] ) )
                            * tau_
                            * (
                            dot ( vel_ts, grad_phi ( i, q, v_var ) )
                            - nu_ * trace ( H_phi ( i, q, v_var ) )
                            )+
#    endif
...
#    ifdef SUPG
                            ...
                            * ( dot ( vel_ts, grad_phi ( i, q, v_var ) ) - nu_ * trace ( H_phi ( i, q, v_var ) ) )
                            ...
                            // incompressibility term
                            + tau_rho_ * inv_rho_ * div_u_k * grad_phi ( i, q, v_var )[v_var]
#    endif
                            )
                            * dJ;
                }
...
#    ifdef SUPG
            hess_vel_ns_[d].clear ( );
            hess_vel_ts_[d].clear ( );
#    endif
...
#    ifdef SUPG
            evaluate_fe_function_hessians ( *prev_newton_sol_, d, hess_vel_ns_[d] );
            evaluate_fe_function_hessians ( *prev_time_sol_, d, hess_vel_ts_[d] );
#    endif
        }
...
#    ifdef SUPG
        grad_p_ns_.clear ( );
        evaluate_fe_function_gradients ( *prev_time_sol_, DIMENSION, grad_p_ns_ );
#    endif

#    ifdef SUPG
        // Compute local matrix.
        const size_t num_q = num_quadrature_points ( );
...
#    ifdef SUPG
    FunctionValues< Vec<DIMENSION, double> > grad_p_ns_; // gradient of pressure at previous newton step
    FunctionValues< Mat<DIMENSION, DIMENSION, double> > hess_vel_ns_[DIMENSION]; // Hessian matrices of velocity at previous Newton step
    FunctionValues< Mat<DIMENSION, DIMENSION, double> > hess_vel_ts_[DIMENSION]; // Hessian matrices of velocity at previous Newton step
#    endif
};
...
class L2NormIntegratorPp : private AssemblyAssistant<DIMENSION, double>
{
  public:
    ...
    L2NormIntegratorPp ( CVector& pp_sol, const std::vector<int>& vars )
...
class H1semiNormIntegratorPp : private AssemblyAssistant<DIMENSION, double>
{
  public:
    ...
    H1semiNormIntegratorPp ( CVector& pp_sol, const std::vector<int>& vars )
    : pp_sol_ ( pp_sol ), vars_ ( vars )
    {
    }
