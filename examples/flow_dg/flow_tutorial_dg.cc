...
static const char* PARAM_FILENAME = "flow_tutorial_dg.xml";
...
int main ( int argc, char** argv )
{
    ...
    std::ofstream info_log ( "_info_log" );
    LogKeeper::get_log ( "info" ).set_target ( &std::cout );
    std::ofstream debug_log ( "_debug_log" );
...
#ifdef WITH_ILUPP
    precond_.InitParameter ( space_, 1., 1 );
    linsolver_->SetupPreconditioner ( precond_ );
#endif
