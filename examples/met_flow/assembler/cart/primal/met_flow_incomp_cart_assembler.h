...
#ifndef MET_FLOW_INCOMP_CART_ASSEMBLER_H
#    define MET_FLOW_INCOMP_CART_ASSEMBLER_H

///
/// \brief Assembler class for incompressible Navier Stokes equations in cartesian coordinates
///
...
class MetFlowIncompCartAssembler : public virtual MetFlowIncompAssembler<DIM, DataType>
{
  public:

    MetFlowIncompCartAssembler ( );
    ...
    virtual void operator() ( const Element<DataType>& element, const Quadrature<DataType>& quadrature, LocalMatrix& lm )
    {
        ...
        MetFlowIncompCartAssembler<DIM, DataType>::assemble_local_matrix ( element, lm );
    }
    ...
    {
        MetFlowIncompAssembler<DIM, DataType>::initialize_for_element ( element, quadrature );
        MetFlowIncompCartAssembler<DIM, DataType>::assemble_local_vector ( element, lv );
    }

    virtual void operator() ( const Element<DataType>& element, const Quadrature<DataType>& quadrature, DataType& ls )
    {
        MetFlowIncompAssembler<DIM, DataType>::initialize_for_element ( element, quadrature );
        MetFlowIncompCartAssembler<DIM, DataType>::assemble_local_scalar ( element, ls );
    }
    ...
    virtual void operator() ( const Element<DataType>& element, int facet_number, const Quadrature<DataType>& quadrature, LocalVector& lv )
    {
        ...
        MetFlowIncompCartAssembler<DIM, DataType>::assemble_local_scalar_boundary ( element, facet_number, lv );
    }
